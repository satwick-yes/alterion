...
def index_vector_memory_from_update(memory_update: dict) -> None:
    try:
            ...
            if isinstance(items, dict):
                for key, val_obj in items.items():
                    ...
                    if isinstance(val_obj, dict) and "value" in val_obj:
                        val = val_obj["value"]
                    elif isinstance(val_obj, (str, int, float)):
                        val = str(val_obj)
                    if val:
                        formatted_fact = f"User's {key.replace('_', ' ')} is {val}"
                        vector_memory_db.add_memory(formatted_fact, category)
    except Exception as e:
        print(f"[Memory] ⚠️ Vector indexing failed: {e}")
...
def update_memory(memory_update: dict) -> dict:
    ...
    if _recursive_update(memory, memory_update):
        ...
        # Run indexing in a background thread or call it directly since embedding generation is fast
        import threading
        threading.Thread(target=index_vector_memory_from_update, args=(memory_update,), daemon=True).start()
    return memory
