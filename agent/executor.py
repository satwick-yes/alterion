...
def _call_tool(tool: str, parameters: dict, speak: Callable | None, companion_name: str = "") -> str:
    from core.integration_gateway import integration_gateway
    if not integration_gateway.verify_tool_permission(companion_name, tool):
        raise PermissionError(f"Action blocked: Companion '{companion_name}' is not authorized to use tool '{tool}'")
...
class AgentExecutor:
    ...
    def execute(
        ...
        task_id = getattr(threading.current_thread(), "task_id", str(uuid.uuid4())[:8])
        state_manager.register_task(task_id, goal)
        state_manager.set_agent_state(AgentState.EXECUTING)

        # 2. Check if complex task to spawn parallel worker bots
        if multi_agent_framework.check_is_complex(goal):
            state_manager.update_task_status(task_id, TaskState.RUNNING)
            try:
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
                result = loop.run_until_complete(
                    multi_agent_framework.execute_complex_task(task_id, goal, companion)
                )
                loop.close()
                state_manager.update_task_status(task_id, TaskState.COMPLETED)
                if speak:
                    speak(result)
                return result
            except Exception as e:
                state_manager.update_task_status(task_id, TaskState.FAILED, str(e))
                if speak:
                    speak(f"Sir, the parallel bot workers encountered a failure: {e}")
                raise
        ...
        while True:
            ...
            if not steps:
                ...
                state_manager.update_task_status(task_id, TaskState.FAILED, msg)
                if speak: speak(msg)
                return msg
            ...
            for step in steps:
                ...
                while attempt <= 3:
                    ...
                    try:
                        ...
                        error_msg = str(e)
                        state_manager.update_task_step_status(task_id, step_num, f"failed: {error_msg}")
            ...
            if success:
                state_manager.update_task_status(task_id, TaskState.COMPLETED)
                state_manager.set_agent_state(AgentState.IDLE)
