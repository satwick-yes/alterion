...
def _is_safe_path(target: Path) -> bool:
    """Checks if the path is safely within the user's home directory."""
    try:
        home = Path.home().resolve()
        target_resolved = target.resolve()
        return target_resolved.is_relative_to(home)
    except Exception:
        return False
...
def delete_file(path: str, confirm: bool = True) -> str:
    ...
    try:
        ...
        if not _is_safe_path(target):
            return f"Action blocked: Path '{path}' is outside your user directory."
            
        if not target.exists():
            return f"Not found: {path}"

        if confirm:
            return f"Action blocked. Ask the user for explicit permission to delete '{target.name}', then call this tool again with confirm=False."
...
def write_file(path: str, content: str, append: bool = False, overwrite: bool = False) -> str:
    ...
    try:
        ...
        if target.exists() and not append and not overwrite:
            return f"Action blocked. The file '{target.name}' already exists. Ask the user for permission to overwrite it, then call this tool again with overwrite=True."
...
def file_controller(
    ...
    try:
        if action == "list":
            ...
            full = _full_path(path, name)
            result = delete_file(full, confirm=parameters.get("confirm", True))
