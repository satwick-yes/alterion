...
def _focus_window(title: str) -> str:
    ...
    if os_name == "windows":
        try:
            vbs = _BASE / "tools" / "focus_and_hotkey.vbs"
            result = subprocess.run(
                ["cscript", "//Nologo", str(vbs), title, ""],
...
def _screen_find(description: str) -> tuple[int, int] | None:
    """Use Gemini Vision (primary) or OpenRouter vision (fallback) to locate a UI element."""
    _require_pyautogui()
    import re as _re
    ...
    prompt = (
        ...
        f"Reply ONLY with the center pixel coordinates in the format: x,y "
        f"(two integers separated by a comma). "
        f"If the element is not visible, reply only with: NOT_FOUND"
    )
    ...
    try:
        ...
        _key = _json.loads(_cfg.read_text(encoding="utf-8")).get("gemini_api_key", "").strip()
        if _key:
            ...
            coords = _parse(_resp.text.strip())
            if coords:
                print(f"[ComputerControl] 🔍 Gemini found '{description}' at {coords}")
                return coords
    except Exception as e:
    ...
    try:
        ...
        coords = _parse(text)
        if coords:
            print(f"[ComputerControl] 🔍 OpenRouter found '{description}' at {coords}")
            return coords
    except Exception as e:
        print(f"[ComputerControl] ⚠️ screen_find fallback failed: {e}")
...
def computer_control(
    ...
    try:
        ...
        if action == "hotkey":
            ...
            time.sleep(0.3)  # ensure focused window is ready before sending keys
            return _hotkey(*keys)
        ...
        if action == "focus_and_hotkey":
            # Atomically focus a window and send a hotkey via VBScript (bypasses Python sandbox)
            win_title = params.get("title", "")
            hotkeys   = params.get("keys", "")
            if not win_title or not hotkeys:
                return "focus_and_hotkey requires 'title' and 'keys' params"
            # Convert 'ctrl+shift+v' -> '^+v' (WScript.Shell SendKeys format)
            key_map = {"ctrl": "^", "shift": "+", "alt": "%"}
            parts = hotkeys.lower().split("+")
            send_str = ""
            for p in parts:
                p = p.strip()
                send_str += key_map.get(p, p if len(p) == 1 else "{" + p + "}")
            vbs = _BASE / "tools" / "focus_and_hotkey.vbs"
            result = subprocess.run(
                ["cscript", "//Nologo", str(vbs), win_title, send_str],
                capture_output=True, text=True, timeout=8
            )
            if "NOT_FOUND" in result.stdout:
                return f"Could not find window '{win_title}' - is the app open?"
            return f"Sent '{hotkeys}' to '{win_title}'"
