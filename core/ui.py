...
class HudCanvas(QWebEngineView):
    ...
    def _handle_permission(self, securityOrigin, feature):
        if feature in (self.page().Feature.MediaAudioCapture, 
                       self.page().Feature.MediaVideoCapture, 
                       self.page().Feature.MediaAudioVideoCapture):
            self.page().setFeaturePermission(securityOrigin, feature, self.page().PermissionPolicy.PermissionGrantedByUser)
    ...
    def _step(self):
        ...
        js_code = f"if (window.setVaniState) window.setVaniState('{target_state}', {self._mic_level});"
...
class LogWidget(QTextEdit):
    ...
    def _next(self):
        ...
        elif tl.startswith("vani:"): self._tag = "ai"
...
class FileDropZone(QWidget):
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for VANI", str(Path.home()),
            "All Files (*.*);;"
...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(_lbl("Configure Vani before first boot.", 9, color=C.PRI_DIM))
        layout.addSpacing(6)
...
class MainWindow(QMainWindow):
    ...
    def _build_header(self) -> QWidget:
        ...
        lay.addWidget(_badge("Vani", C.PRI_DIM))
        lay.addStretch()
        ...
        mid.addWidget(title)
        sub = QLabel("Your personal AI — built by Satwick")
    ...
    def _build_footer(self) -> QWidget:
        ...
        lay.addStretch()
        lay.addWidget(_fl("Vani  ·  Built by Satwick"))
        lay.addStretch()
        lay.addWidget(_fl("© Satwick Industries", C.PRI_DIM))
        return w
    ...
    def _on_file_selected(self, path: str):
        ...
        self._file_hint.setText(f"{icon}  {p.name}  ·  {size}  ·  Tell VANI what to do with it")
    ...
    def _on_setup_done(self, keys: dict, os_name: str):
        ...
        if self._overlay:
            ...
            self._overlay = None
        self._apply_state("LISTENING")
        self._log.append_log(f"SYS: Vani online. OS={os_name.upper()}. Ready!")
